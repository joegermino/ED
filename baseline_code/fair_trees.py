...
class FairDecisionTreeClassifier():
    ...
    def fit(self, X="X", y="y", s="s", **kwargs):
        ...
        if self.oob_pruning and self.criterion=="scaff":
            ...
            if isinstance(auc_s, list):
                best_score = (1-self.orthogonality)*auc_y - self.orthogonality*max(auc_s)
            else:
            ...
            while not stop_flag:
                ...
                if len(paths_leaves)>1:
                    for i in range(len(paths_leaves)):
                        ...
                        auc_s = sensitive_auc_score(self.s_validation, y_prob) 
                        if isinstance(auc_s, list):
                            new_score = (1-self.orthogonality)*auc_y - self.orthogonality*max(auc_s)
                        else:
